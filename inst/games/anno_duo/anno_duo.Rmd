...
* Here's an interesting question: in Turn 4, Moby was trying to move `B3 => B1 => B4` but ran into Chris in `B1`. Mobes wins the battle and, the way I have it now, he proceeds to `B4` (his original destination) leaving `B1` (the site of the battle) unoccupied. I guess this could make sense theoretically, but it seems a little weird. Or maybe, if he had wanted to leave anybody in `B1` then he would have to specify that during the retreat phase? Thoughts?
* Also, I'm pretty sure Eric and Moby are just passing in the night in `H5` and `H6`. Is that right? If so... this shouldn't be legal, right? They have to fight?
